...
const LIBRES = Array.from({ length: 7 }, (_, i) => ({
  x: r3(66 + rand(i * 3 + 40) * 30),
  y: r3(16 + rand(i * 3 + 41) * 62),
  r: r3(3 + rand(i * 3 + 42) * 3.4),
  teinte: Math.floor(rand(i * 3 + 43) * PALETTE.length),
  delai: r3(rand(i * 3 + 44) * 4),
  duree: r3(3.5 + rand(i * 3 + 45) * 3),
}));
...
export function AksanticMark({
  ...
  const faces = grille
    .map((c, i) => {
      ...
      const ecart = d * d * 14;
      // On fixe 3 décimales à TOUTE valeur numérique injectée dans le SVG.
      // Sans ça, serveur et client arrondissent différemment un float à 16
      // décimales, et React lève une erreur d'hydratation (visible sur /login).
      const r3 = (n: number) => Number(n.toFixed(3));
      return {
        key: i,
        cx: r3(CX + R * x3 + c.jitterX * ecart),
        cy: r3(CY - R * y3 + c.jitterY * ecart),
        r: r3((2.6 + 1.7 * z3) * (1 - d * 0.35)),
