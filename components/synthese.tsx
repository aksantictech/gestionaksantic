...
import { toCdf, fmt, totalFacture, payeCdf, resteCdf, etatFacture, today } from "@/lib/money";
...
export function Synthese({ d, peutEcrire }: P) {
  ...
  const k = useMemo(() => {
    ...
    return {
      facture, encaisse, attente, sorties, retard,
  ...
  const mensuel = useMemo(() => {
    ...
    d.depenses.forEach((x) => {
      trouve(x.date) && (trouve(x.date)!.depense += toCdf(x.montant, x.devise, x.taux));
    });
    return mois;
  }, [d]);
